...
type CreateMarketHandler = (
  ctx: Ctx,
  args: any,
  baseData: BaseCallData,
  block: SubstrateBlock
) => Promise<PredictionMarketCreation>
...
type TransferHandler = (
  ...
  args: any,
  baseData: BaseCallData
) => Promise<PredictionMarketAssetTransfer>
...
export async function processPredictionMarketCall(ctx: Ctx, item: any, block: SubstrateBlock) {
  ...
  try {
    const result = await handler(
      ctx,
      { ...args, operationType: effectiveCallName },
      baseData,
      block
    )
